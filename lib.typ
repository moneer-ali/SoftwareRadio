...
#let create-title-page(
  ...
  // Logo
  if logo1 != none or logo2 != none {
    // v(-1em)
    {
      let y = 0.7cm
      pad(x: -1.2cm, top: -y, bottom: y, grid(
        columns: (1fr, 1fr),
        align(left, if logo2 != none { logo2 }),
        //
        align(right, if logo1 != none { logo1 }),
      ))
    }
  }
  ...
  if department != none {
    align(center, text(1.5em, department))
  }
  ...
  if course != "" {
    ...
    align(center, text(1.3em, course))
  }
  ...
  if advisers.len() > 0 {
    align(center, text(1.2em, smallcaps[Submitted to]))
    v(0.8em)
    ...
    grid(
      ...
      ..advisers.map(adviser => align(
        ...
        text(
          ...
          [
            *#adviser.name* \
            #if "email" in adviser { adviser.email }
          ],
        ),
      ))
    )
...
#let academic-document(
  logo1: image("logos/logo_uni.png", height: 100pt), //"logos/logo.png",
  logo2: image("logos/logo_eng.png", height: 100pt), //"logos/logo.png",
  ...
  logo-width: 7%,
  document-type: none,
  department: "Electronics & Electrical Communications Department",
  course-name: none,
  course-code: none,
  title: "",
  abstract: [],
